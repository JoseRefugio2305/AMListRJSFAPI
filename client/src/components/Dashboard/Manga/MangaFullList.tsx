...
export default function MangaFullList() {
   const {
      mangas,
      totalMangas,
      filtersParam,
   ...
   const handleSort = (event: DataTableSortEvent) => {
      setFiltersParam((prev) => {
         return {
            ...
            orderField: (() => {
               const sortField =
                  event.sortField === "key_manga" ? "key" : event.sortField;
   ...
   return (
      <>
         ...
         <section className="w-full grid gap-5 shadow-2xl md:rounded-2xl rounded-none dark:bg-gray-700 p-6 mt-4 mb-1">
            <h2 className="text-xl font-bold">{totalMangas} Resultado(s)</h2>
         </section>
         <DataTable
            value={mangas}
            lazy
            ...
            currentPageReportTemplate={`Mostrando ${
               totalMangas > 0 ? lazyState.first + 1 : totalMangas
            } a ${Math.min(
               lazyState.rows + lazyState.first,
               totalMangas
            )} manga(s) de ${totalMangas} resultados`}
            first={lazyState.first}
            totalRecords={totalMangas}
            onPage={handlePage}
            onSort={handleSort}
            ...
            rowHover={true}
            emptyMessage={
               <NotResultsFound message="No Se Encontraron Mangas Que Concuerden con la Búsqueda." />
            }
