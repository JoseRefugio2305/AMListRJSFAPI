...
import { updateAllAnimes } from "../../../services/dashboardAnimeServides";
...
export default function ModalUpdAll({ setOpenModal }: ModalProps) {
   ...
   const fetchAnimes = () => {
      getIncompleteAnimes({ limit: 1, page: 1 }, 1)
         .then((resp) => {
            if (!resp.is_success) {
               showToast({
                  severity: "error",
            ...
            setTotalToAct(resp.totalAnimes ?? 0);
            setLoading(false);
         })
         .catch((error) => {
            console.error(error);
         ...
         <ModalBody>
            <div className="space-y-6 items-center justify-center flex flex-col">
               {totalToAct > 0 ? (
                  ...
                  <h2 className="text-2xl font-bold text-center">
                     Actualmente No Hay Animes Para Actualizar.
                  </h2>
               )}
            </div>
         </ModalBody>
